...
build_docker() {
    ...
    docker build -f spec/docker/Dockerfile . -t verify_beryllium
}
...
spawn_single_container() {
    conf="$1"
    id=$(docker run -d -e CERTORAKEY -e BRANCH -e ALL_SPEC -v $(pwd)/build:/root/build:ro verify_beryllium $conf $SANITY $TYPECHECK_ONLY)
    echo "Spawned $id"
    ids="$ids $id"
}
