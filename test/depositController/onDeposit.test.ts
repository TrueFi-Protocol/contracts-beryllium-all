...
describe('DepositController.onDeposit', () => {
  ...
  it('only allowed can deposit', async () => {
    const { wallet, other, portfolio, mockLenderVerifier, token } = await loadFixture(automatedLineOfCreditFixture)
    ...
    await expect(portfolio.deposit(1, other.address)).to.be.revertedWith('AutomatedLineOfCredit: Operation not allowed')
