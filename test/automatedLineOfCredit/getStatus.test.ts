...
describe('AutomatedLineOfCredit.getStatus', () => {
  ...
  it('is Open after portfolio creation', async () => {
    ...
    expect(await portfolio.getStatus()).to.equal(AutomatedLineOfCreditStatus.Open)
  })
  ...
  it('is Full when deposited amount exceeds max size', async () => {
    ...
    expect(await portfolio.getStatus()).to.equal(AutomatedLineOfCreditStatus.Full)
  })

  it('is Closed when end date elapses', async () => {
    const { portfolio, timeTravel } = await loadFixture()
    await timeTravel(YEAR)
    expect(await portfolio.getStatus()).to.equal(AutomatedLineOfCreditStatus.Closed)
  })
})
