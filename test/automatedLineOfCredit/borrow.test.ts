...
describe('AutomatedLineOfCredit.borrow', () => {
  ...
  it('reverts if paused', async () => {
    ...
    await expect(portfolio.borrow(parseUSDC(amount))).to.be.revertedWith('Pausable: paused')
  })

  it('does not allow non-borrower to borrow', async () => {
    const { portfolio, other } = await loadFixture()
    await expect(portfolio.connect(other).borrow(parseUSDC(amount)))
      .to.be.revertedWith('AutomatedLineOfCredit: Caller is not the borrower')
  })

  it('cannot borrow more funds than available', async () => {
    const { portfolio } = await loadFixture()
    await expect(portfolio.borrow(parseUSDC(amount + 1)))
      .to.be.revertedWith('AutomatedLineOfCredit: Amount exceeds portfolio balance')
  })

  it('cannot borrow zero assets', async () => {
    const { portfolio } = await loadFixture()
    await expect(portfolio.borrow(0))
      .to.be.revertedWith('AutomatedLineOfCredit: Cannot borrow zero assets')
  })
  ...
  it('transfers funds to borrower', async () => {
    ...
    expect(await token.balanceOf(wallet.address)).to.equal(borrowerBalanceBefore.add(parseUSDC(amount)))
  })
  ...
  it('updates portfolio balance properly', async () => {
    ...
    expect(portfolioBalanceAfter).to.equal(portfolioBalanceBefore.sub(parseUSDC(amount)))
    expect(portfolioVirtualBalanceAfter).to.equal(portfolioVirtualBalanceBefore.sub(parseUSDC(amount)))
  ...
  it('updates accruedInterest', async () => {
    ...
    await portfolio.borrow(parseUSDC(amount / 2))
    expect(await portfolio.accruedInterest()).to.closeTo(interest, parseUSDC(1))
  })

  it('cannot borrow past end date', async () => {
    const { portfolio, timeTravel } = await loadFixture()
    await timeTravel(YEAR + DAY)
    await expect(portfolio.borrow(parseUSDC(amount / 2)))
  ...
  it('transfers fee', async () => {
    const { portfolio, token, executeAndSetNextTimestamp, protocolConfig, protocolTreasury } = await loadFixture()
  ...
  it('borrow limit includes fees', async () => {
    const { portfolio, executeAndSetNextTimestamp, protocolConfig } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await executeAndSetNextTimestamp(portfolio.updateAndPayFee(), YEAR / 2)
    await expect(portfolio.borrow(parseUSDC(amount))).to.be.revertedWith('AutomatedLineOfCredit: Amount exceeds portfolio balance')
  })

  it('emits FeePaid event', async () => {
    const { portfolio, executeAndSetNextTimestamp, protocolConfig, protocolTreasury } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await executeAndSetNextTimestamp(portfolio.updateAndPayFee(), YEAR / 2)
    await expect(portfolio.borrow(parseUSDC(1))).to.emit(portfolio, 'FeePaid').withArgs(protocolTreasury, parseUSDC(amount * 0.05))
  })
})
