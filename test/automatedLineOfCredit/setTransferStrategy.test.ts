...
describe('AutomatedLineOfCredit.setTransferController', () => {
  ...
  it('can only be set by a strategy admin', async () => {
    const { portfolio, wallet, other, CONTROLLER_ADMIN_ROLE } = await loadFixture()
    const newTransferController = await new OpenTransferController__factory(wallet).deploy()
  ...
  it('prevents from setting the same strategy', async () => {
    ...
    await expect(portfolio.setTransferController(transferController.address))
      .to.be.revertedWith('AutomatedLineOfCredit: New transfer controller needs to be different')
  })

  it('emits event', async () => {
    const { portfolio, setTransferController, wallet } = await loadFixture()
    const newTransferController = await new OpenTransferController__factory(wallet).deploy()
    await expect(setTransferController(newTransferController.address)).to.emit(portfolio, 'TransferControllerChanged').withArgs(newTransferController.address)
  })
})
