...
describe('AutomatedLineOfCredit.previewDeposit', () => {
  ...
  describe('default deposit controller', () => {
    ...
    it('returns less shares when portfolio value appreciates', async () => {
      const { portfolio, wallet, deposit, parseShares, borrowAndRepayWithInterest } = await loadFixture()
    ...
    it('reverts when portfolio is closed', async () => {
      ...
      await expect(portfolio.previewDeposit(parseUSDC(amount)))
        .to.be.revertedWith('AutomatedLineOfCredit: Portfolio end date has elapsed')
    })

    it('accounts for protocol fee', async () => {
      const { portfolio, executeAndTimeTravel, deposit, protocolConfig, wallet } = await loadFixture()
  ...
  describe('non-default deposit controller is set', () => {
    it('returns correct amount of shares', async () => {
      const { portfolio, parseShares, mockDepositController } = await loadFixture()
