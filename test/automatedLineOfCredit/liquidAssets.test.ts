...
describe('AutomatedLineOfCredit.liquidAssets', () => {
  ...
  it('increases after repay', async () => {
    ...
    await deposit(wallet, parseUSDC(amount))
    const interest = await borrowAndRepayWithInterest(parseUSDC(amount / 2), YEAR)
    expect(await portfolio.liquidAssets()).to.equal(parseUSDC(amount).add(interest))
  })

  it('is decreased by protocol continuous fee', async () => {
    const { portfolio, wallet, deposit, executeAndTimeTravel, protocolConfig } = await loadFixture()
  ...
  it('is 0 when fees exceed virtualTokenBalance', async () => {
    const { portfolio, deposit, executeAndTimeTravel, wallet, token, protocolConfig } = await loadFixture()
