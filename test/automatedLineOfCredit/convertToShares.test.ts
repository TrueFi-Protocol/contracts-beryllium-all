...
describe('AutomatedLineOfCredit.convertToShares', () => {
  ...
  it('returns less shares when portfolio value appreciates', async () => {
    ...
    const expectedShares = parseUSDC(amount).mul(parseShares(amount)).div(parseUSDC(amount).add(interest))
  ...
  it('accounts for fees', async () => {
    ...
    const expectedShares = parseUSDC(amount).mul(parseShares(amount)).div(parseUSDC(amount * 0.95))
    expect(await portfolio.convertToShares(parseUSDC(amount))).to.equal(expectedShares)
  })
})
