...
describe('AutomatedLineOfCredit.maxWithdraw', () => {
  ...
  it('returns 0 when not allowed by withdraw controller', async () => {
    const { portfolio, deposit, wallet, mockWithdrawController } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
  ...
  it('is limited by liquid assets', async () => {
    ...
    expect(await portfolio.maxWithdraw(wallet.address)).to.equal(parseUSDC(amount).div(2))
  })

  it('returns more with accrued interest', async () => {
    const { portfolio, deposit, wallet, borrowAndRepayWithInterest } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
    const accruedInterest = await borrowAndRepayWithInterest(parseUSDC(amount))
    expect(await portfolio.maxWithdraw(wallet.address)).to.equal(parseUSDC(amount).add(accruedInterest))
  })

  it('includes fee', async () => {
    const { portfolio, wallet, protocolConfig, executeAndTimeTravel, deposit } = await loadFixture()
