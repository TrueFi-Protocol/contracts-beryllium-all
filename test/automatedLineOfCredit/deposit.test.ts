...
describe('AutomatedLineOfCredit.deposit', () => {
  ...
  it('reverts when portfolio is receiver', async () => {
    ...
    await expect(portfolio.deposit(amount, portfolio.address))
      .to.be.revertedWith('AutomatedLineOfCredit: Portfolio cannot be the receiver')
  })

  it('reverts for 0 deposit', async () => {
    const { deposit, wallet } = await loadFixture()
    await expect(deposit(wallet, parseUSDC(0)))
      .to.be.revertedWith('AutomatedLineOfCredit: Operation not allowed')
  })

  it('distributes shares to receiver', async () => {
    const { portfolio, other, wallet, parseShares, token } = await loadFixture()
  ...
  it('distributes fewer tokens when portfolio value appreciates', async () => {
    const { portfolio, wallet, other, token, deposit, parseShares, borrowAndRepayWithInterest } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
    const interest = await borrowAndRepayWithInterest(parseUSDC(amount), YEAR / 4)
    const newPortfolioValue = parseUSDC(amount).add(interest)

    await token.mint(other.address, parseUSDC(amount))
    const lpTokensMinted = parseUSDC(amount).mul(parseShares(amount)).div(newPortfolioValue)
  ...
  it('updates accruedInterest', async () => {
    const { portfolio, wallet, deposit, timeTravel, calculateInterest } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
    ...
    await deposit(wallet, parseUSDC(amount))
    expect(await portfolio.accruedInterest()).to.be.closeTo(interest, parseUSDC(1))
  })
  ...
  it('cannot deposit past end date', async () => {
    ...
    await expect(deposit(wallet, parseUSDC(amount)))
      .to.be.revertedWith('AutomatedLineOfCredit: Portfolio end date has elapsed')
  })

  it('cannot deposit past max size', async () => {
    const { other, maxSize, deposit } = await loadFixture()
    await expect(deposit(other, parseUSDC(maxSize).add(1)))
      .to.be.revertedWith('AutomatedLineOfCredit: Operation would cause portfolio to exceed max size')
  })
  ...
  it('cannot deposit after interest exceeds max size', async () => {
    ...
    await portfolio.borrow(parseUSDC(amount))
    await timeTravel(YEAR / 2)
    await expect(deposit(wallet, parseUSDC(1)))
      .to.be.revertedWith('AutomatedLineOfCredit: Operation would cause portfolio to exceed max size')
  })

  it('can deposit when somebody withdraws below max size', async () => {
    const { portfolio, wallet, deposit, withdraw, parseShares } = await loadFixture()
    await portfolio.setMaxSize(parseUSDC(amount))
  ...
  it('calls onDeposit with correct arguments', async () => {
    const { deposit, wallet, other, mockDepositController } = await loadFixture()
    const strategyContract = await mockDepositController({ onDeposit: { shares: 1 } })
    await deposit(wallet, parseUSDC(amount), other)
    expect('onDeposit').to.be.calledOnContractWith(strategyContract, [wallet.address, parseUSDC(amount), other.address])
  })

  it('returns the number of shares minted', async () => {
    const { portfolio, wallet, token, parseShares } = await loadFixture()
    await token.approve(portfolio.address, parseUSDC(amount))
    expect(await portfolio.callStatic.deposit(parseUSDC(amount), wallet.address)).to.equal(parseShares(amount))
  })
  ...
  it('emits a Deposit event', async () => {
    ...
    await expect(deposit(wallet, parseUSDC(amount)))
      ...
      .withArgs(wallet.address, wallet.address, parseUSDC(amount), parseShares(amount))
  })

  it('transfers fee', async () => {
    const { token, wallet, deposit, protocolConfig, protocolTreasury, getTxTimestamp } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    const txTimestamp = await getTxTimestamp(deposit(wallet, parseUSDC(amount)))
  ...
  it('mints correct amount with fee', async () => {
    const { portfolio, token, other, parseShares, wallet, deposit, borrowAndSetNextTimestamp, protocolConfig } = await loadFixture()
    await token.mint(other.address, parseUSDC(amount * 2))
    await token.connect(other).approve(portfolio.address, parseUSDC(amount * 2))
    await deposit(wallet, parseUSDC(amount))
    await protocolConfig.setProtocolFeeRate(1000)
    const interest = await borrowAndSetNextTimestamp(parseUSDC(amount), YEAR / 2)
    const fee = parseUSDC(amount).add(interest).div(20)
    await portfolio.connect(other).deposit(parseUSDC(amount).add(interest).sub(fee), other.address)
    expect(await portfolio.balanceOf(other.address)).to.equal(parseShares(amount))
  })

  it('liquid assets are less than fee', async () => {
    const { portfolio, token, wallet, deposit, protocolConfig, protocolTreasury, getTxTimestamp, calculateInterest } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
    await protocolConfig.setProtocolFeeRate(1000)
    const txTimestamp = await getTxTimestamp(portfolio.borrow(parseUSDC(amount)))
    const interest = await calculateInterest(parseUSDC(amount), YEAR / 2)
    ...
    expect(await portfolio.unpaidFee()).equal(parseUSDC(amount).add(interest).div(20).sub(1))
  })

  it('emits FeePaid event', async () => {
    const { portfolio, deposit, wallet, parseShares, protocolConfig, protocolTreasury, getTxTimestamp } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    const txTimestamp = await getTxTimestamp(deposit(wallet, parseShares(amount)))
    await expect(deposit(wallet, 1, wallet, txTimestamp + YEAR / 2)).to.emit(portfolio, 'FeePaid').withArgs(protocolTreasury, parseUSDC(amount * 0.05))
  })

  it('transfers amount specified in strategy', async () => {
    const { wallet, parseShares, portfolio, deposit, token, mockDepositController } = await loadFixture()
  ...
  describe('whitelist deposit strategy', () => {
    it('does not allow deposit when not whitelisted', async () => {
      const { other, portfolio, deposit, whitelistDepositController } = await loadFixture()
      await portfolio.setDepositController(whitelistDepositController.address)
      await expect(deposit(other, parseUSDC(amount)))
        .to.be.revertedWith('AutomatedLineOfCredit: Operation not allowed')
    })

    it('allows deposit when whitelisted', async () => {
      const { other, portfolio, deposit, whitelistDepositController, parseShares, token } = await loadFixture()
      await portfolio.setDepositController(whitelistDepositController.address)
