...
describe('AutomatedLineOfCredit.maxRedeem', () => {
  ...
  it('is limited by withdraw controller', async () => {
    const { portfolio, deposit, wallet, parseShares, mockWithdrawController } = await loadFixture()
  ...
  it('is not affected by fee', async () => {
    const { portfolio, wallet, protocolConfig, executeAndTimeTravel, mint, parseShares } = await loadFixture()
