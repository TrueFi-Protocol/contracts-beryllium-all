...
describe('AutomatedLineOfCredit.previewRedeem', () => {
  ...
  describe('default withdraw controller', () => {
    ...
    it('returns correct amount', async () => {
      ...
      const claimableAssets = await portfolio.convertToAssets(parseShares(amount))
      expect(await portfolio.previewRedeem(parseShares(amount))).to.equal(claimableAssets)
    })
  ...
  describe('non-default withdraw controller is set', () => {
    it('does not revert when Operation not allowed', async () => {
      const { portfolio, parseShares, deposit, wallet, mockWithdrawController } = await loadFixture()
      const amount = 1
    ...
    it('returns correct amount of assets', async () => {
      const { portfolio, deposit, parseShares, wallet, mockWithdrawController } = await loadFixture()
      const amount = 1
      await deposit(wallet, parseUSDC(amount))
