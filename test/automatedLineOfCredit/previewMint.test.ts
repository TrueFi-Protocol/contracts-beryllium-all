...
describe('AutomatedLineOfCredit.previewMint', () => {
  ...
  it('returns increased asset amount when portfolio value appreciates', async () => {
    const { portfolio, deposit, wallet, borrowAndRepayWithInterest, parseShares } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
    const interest = await borrowAndRepayWithInterest(parseUSDC(amount), YEAR / 2)
    expect(await portfolio.previewMint(parseShares(amount))).to.equal(parseUSDC(amount).add(interest))
  })
  ...
  it('rounds up if necessary when portfolio value appreciates', async () => {
    const { portfolio, wallet, deposit, borrowAndRepayWithInterest, parseShares } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
    const interest = await borrowAndRepayWithInterest(parseUSDC(amount), YEAR / 2)
  ...
  it('accounts for fees', async () => {
    const { portfolio, executeAndTimeTravel, deposit, protocolConfig, wallet, parseShares } = await loadFixture()
  ...
  it('uses controller\'s previewMint when present', async () => {
    const { portfolio, mockDepositController, parseShares } = await loadFixture()
    await mockDepositController({ onMint: { assets: parseUSDC(50) } })
    expect(await portfolio.previewMint(parseShares(amount))).to.equal(parseUSDC(50))
  })
})
