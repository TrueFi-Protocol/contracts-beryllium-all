...
describe('AutomatedLineOfCredit.previewWithdraw', () => {
  ...
  it('does not revert when withdraw not allowed', async () => {
    const { portfolio, mockWithdrawController, parseShares } = await loadFixture()
  ...
  it('returns less shares when portfolio value accrues', async () => {
    const { portfolio, parseShares, borrowAndRepayWithInterest } = await loadFixture()
  ...
  it('gets overwritten by withdraw controller', async () => {
    const { portfolio, deposit, mockWithdrawController, parseShares, wallet } = await loadFixture()
