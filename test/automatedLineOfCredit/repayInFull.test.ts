...
describe('AutomatedLineOfCredit.repayInFull', () => {
  ...
  it('transfers funds to portfolio', async () => {
    const { portfolio, token, wallet, borrowAndSetNextTimestamp } = await loadFixture()
    ...
    expect(await token.balanceOf(wallet.address)).to.equal(borrowerBalanceBefore.sub(repaidAmount))
    expect(await portfolio.virtualTokenBalance()).to.equal(virtualTokenBalanceBefore.add(repaidAmount))
    expect(await token.balanceOf(portfolio.address)).to.equal(await portfolio.virtualTokenBalance())
  })
  ...
  it('correctly calculates interest owed', async () => {
    ...
    expect(await token.balanceOf(portfolio.address)).to.equal(parseUSDC(amount).add(interest))
  })
  ...
  it('reverts if paused', async () => {
    ...
    await expect(portfolio.repayInFull()).to.be.revertedWith('Pausable: paused')
  })
  ...
  it('reverts if caller is not borrower', async () => {
    ...
    await token.connect(wallet).approve(portfolio.address, parseUSDC(amount * 2))

    await expect(portfolio.connect(other).repayInFull()).to.be.revertedWith('AutomatedLineOfCredit: Caller is not the borrower')
  })
  ...
  it('emits Repaid event', async () => {
    ...
    await token.approve(portfolio.address, parseUSDC(amount * 2))
    const interest = await borrowAndSetNextTimestamp(parseUSDC(amount), YEAR / 2)
    await expect(portfolio.repayInFull()).to.emit(portfolio, 'Repaid').withArgs(parseUSDC(amount).add(interest))
  })

  it('transfers fee', async () => {
    const { token, protocolConfig, protocolTreasury, portfolio, executeAndSetNextTimestamp, calculateInterest } = await loadFixture()
  ...
  it('updates unpaidFee', async () => {
    const { token, protocolConfig, executeAndSetNextTimestamp, portfolio } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await token.approve(portfolio.address, parseUSDC(amount * 2))
    await executeAndSetNextTimestamp(portfolio.borrow(parseUSDC(amount)), YEAR / 2)
  ...
  it('emits FeePaid event', async () => {
    const { token, protocolConfig, executeAndSetNextTimestamp, portfolio, protocolTreasury, calculateInterest } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await token.approve(portfolio.address, parseUSDC(amount * 2))
    await executeAndSetNextTimestamp(portfolio.borrow(parseUSDC(amount)), YEAR)
    const interest = await calculateInterest(parseUSDC(amount), YEAR)
  ...
  it('correctly calculates fee after repay', async () => {
    const { executeAndTimeTravel, protocolConfig, getTxTimestamp, portfolio, token, setNextBlockTimestamp, calculateInterest } = await loadFixture()
    await token.approve(portfolio.address, parseUSDC(amount * 2))
    ...
    expect(await portfolio.getFee()).to.equal((parseUSDC(amount).add(interest)).div(10))
  })
})
