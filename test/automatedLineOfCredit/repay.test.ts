...
describe('AutomatedLineOfCredit.repay', () => {
  ...
  it('transfers funds to portfolio', async () => {
    ...
    expect(portfolioBalanceAfter).to.equal(portfolioBalanceBefore.add(parseUSDC(amount)))
    expect(portfolioVirtualBalanceAfter).to.equal(portfolioVirtualBalanceBefore.add(parseUSDC(amount)))
    expect(portfolioVirtualBalanceAfter).to.equal(portfolioBalanceAfter)
    expect(borrowerBalanceAfter).to.equal(borrowerBalanceBefore.sub(parseUSDC(amount)))
  })
  ...
  it('decreases borrowedAmount when repaying over interest', async () => {
    ...
    expect(await portfolio.borrowedAmount()).to.equal(parseUSDC(amount / 2).add(interest))
  })
  ...
  it('can repay past end date', async () => {
    ...
    await token.approve(portfolio.address, parseUSDC(amount / 2))
    const interest = await borrowAndSetNextTimestamp(parseUSDC(amount), YEAR + DAY)
    await portfolio.repay(parseUSDC(amount / 2))
  ...
  it('reverts if paused', async () => {
    ...
    await expect(portfolio.repay(parseUSDC(amount))).to.be.revertedWith('Pausable: paused')
  })
  ...
  it('reverts if caller is not borrower', async () => {
    ...
    await portfolio.borrow(parseUSDC(amount))
    await token.approve(portfolio.address, parseUSDC(amount))
    await expect(portfolio.connect(other).repay(parseUSDC(amount))).to.be.revertedWith('AutomatedLineOfCredit: Caller is not the borrower')
  })
  ...
  it('updates last protocol fee rate', async () => {
    const { portfolio, protocolConfig, token, protocolFeeRate } = await loadFixture()
    await portfolio.borrow(parseUSDC(amount))
  ...
  it('reverts when repaying 0', async () => {
    ...
    await portfolio.borrow(parseUSDC(amount))
    await token.approve(portfolio.address, parseUSDC(amount))
    await expect(portfolio.repay(0)).to.be.revertedWith('AutomatedLineOfCredit: Repayment amount must be greater than 0')
  })

  it('transfers fee', async () => {
    const { token, protocolConfig, protocolTreasury, executeAndSetNextTimestamp, portfolio, calculateInterest } = await loadFixture()
    ...
    expect(await token.balanceOf(protocolTreasury)).to.equal(parseUSDC(amount).add(interest).div(10))
  })

  it('updates unpaidFee', async () => {
    const { token, protocolConfig, portfolio, executeAndSetNextTimestamp, calculateInterest } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await token.approve(portfolio.address, parseUSDC(amount))
    await executeAndSetNextTimestamp(portfolio.borrow(parseUSDC(amount)), YEAR)
    const interest = await calculateInterest(parseUSDC(amount), YEAR)
    ...
    expect(await portfolio.unpaidFee()).to.equal(parseUSDC(amount).add(interest).div(10).sub(parseUSDC(1)))
  })

  it('decreases accruedInterest when repaying below fee', async () => {
    const { token, protocolConfig, executeAndSetNextTimestamp, portfolio, calculateInterest } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await token.approve(portfolio.address, parseUSDC(amount))
    await executeAndSetNextTimestamp(portfolio.borrow(parseUSDC(amount)), YEAR)
    const interest = await calculateInterest(parseUSDC(amount), YEAR)
    ...
    expect(await portfolio.accruedInterest()).to.equal(interest.sub(parseUSDC(1)))
    expect(await portfolio.borrowedAmount()).to.equal(parseUSDC(amount))
  })
  ...
  it('emits FeePaid event', async () => {
    const { token, protocolConfig, executeAndTimeTravel, portfolio, protocolTreasury } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await portfolio.borrow(parseUSDC(amount))
    await executeAndTimeTravel(token.approve(portfolio.address, parseUSDC(amount)), YEAR)
  ...
  it('cannot repay more than total debt', async () => {
    ...
    await token.approve(portfolio.address, parseUSDC(amount * 2))
    const interest = await borrowAndSetNextTimestamp(parseUSDC(amount), YEAR / 2)
    await expect(portfolio.repay(parseUSDC(amount).add(interest).add(1))).to.be.revertedWith('AutomatedLineOfCredit: Amount must be less than total debt')
  })
})
