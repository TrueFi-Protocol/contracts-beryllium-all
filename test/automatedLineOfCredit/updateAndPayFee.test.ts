...
describe('AutomatedLineOfCredit.updateAndPayFee', () => {
  ...
  it('pays fee', async () => {
    const { portfolio, token, protocolConfig, protocolTreasury, executeAndSetNextTimestamp } = await loadFixture()
  ...
  it('updates unpaidFee', async () => {
    const { portfolio, borrowAndSetNextTimestamp, protocolConfig } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    const interest = await borrowAndSetNextTimestamp(parseUSDC(amount), YEAR)
    await portfolio.updateAndPayFee()
    expect(await portfolio.unpaidFee()).to.equal(parseUSDC(amount).add(interest).div(10))
  })
