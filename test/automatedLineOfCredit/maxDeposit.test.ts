...
describe('AutomatedLineOfCredit.maxDeposit', () => {
  ...
  describe('default deposit controller', () => {
    ...
    it('decreases when portfolio value appreciates', async () => {
      const { portfolio, wallet, deposit, maxSize, borrowAndRepayWithInterest } = await loadFixture()
      await deposit(wallet, maxSize.div(2))
      const interest = await borrowAndRepayWithInterest(maxSize.div(2), YEAR / 4)
      const newPortfolioValue = maxSize.div(2).add(interest)
      expect(await portfolio.maxDeposit(wallet.address)).to.equal(maxSize.sub(newPortfolioValue))
    })
    ...
    it('includes fee', async () => {
      ...
      expect(await portfolio.maxDeposit(wallet.address)).to.equal(maxSize.div(2).add(fee))
    })
  })
})
