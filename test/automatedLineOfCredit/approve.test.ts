...
describe('AutomatedLineOfCredit.approve', () => {
  ...
  it('sets allowance when not paused', async () => {
    ...
    expect(await portfolio.allowance(wallet.address, other.address)).to.equal(parseShares(amount))
  })

  it('reverts when paused', async () => {
    const { portfolio, other, parseShares } = await loadFixture()
    await portfolio.pause()
    await expect(portfolio.approve(other.address, parseShares(amount)))
