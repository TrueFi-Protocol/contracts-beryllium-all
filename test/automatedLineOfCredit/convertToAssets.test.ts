...
describe('AutomatedLineOfCredit.convertToAssets', () => {
  ...
  it('returns correct value after deposit', async () => {
    ...
    expect(await portfolio.convertToAssets(parseShares(amount))).to.equal(parseUSDC(amount))
  })

  it('returns larger amount when portfolio value appreciates', async () => {
    const { portfolio, parseShares, borrowAndRepayWithInterest, deposit, wallet } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
    const interest = await borrowAndRepayWithInterest(parseUSDC(amount), YEAR)
    expect(await portfolio.convertToAssets(parseShares(amount))).to.equal(parseUSDC(amount).add(interest))
  })
  ...
  it('accounts for fees', async () => {
    ...
    const expectedAssets = parseShares(amount).mul(parseUSDC(amount * 0.95)).div(parseShares(amount))
    expect(await portfolio.convertToAssets(parseShares(amount))).to.equal(expectedAssets)
  })
})
