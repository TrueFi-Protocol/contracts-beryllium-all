...
describe('AutomatedLineOfCredit.totalAssets', () => {
  ...
  it('is decreased by fees after withdraw', async () => {
    const { portfolio, token, other, deposit, withdraw, parseShares } = await loadFixture()
    await token.mint(other.address, parseUSDC(amount))
    await deposit(other, parseUSDC(amount))
    await withdraw(other, parseShares(amount / 2))

    expect(await portfolio.totalAssets()).to.equal(parseUSDC(amount / 2))
    expect(await token.balanceOf(portfolio.address)).to.equal(parseUSDC(amount / 2))
  })

  it('grows over time after borrow', async () => {
    const { portfolio, token, other, deposit, borrowAndSetNextTimestamp, mineBlock } = await loadFixture()
    await token.mint(other.address, parseUSDC(amount))
    await deposit(other, parseUSDC(amount))
    const interest = await borrowAndSetNextTimestamp(parseUSDC(amount), YEAR)
    await mineBlock()
    expect(await portfolio.totalAssets()).to.equal(parseUSDC(amount).add(interest))
  })
  ...
  it('is decreased by accrued fee', async () => {
    const { portfolio, deposit, wallet, protocolConfig, executeAndTimeTravel } = await loadFixture()
  ...
  it('is decreased by unpaidFee', async () => {
    const { portfolio, deposit, wallet, protocolConfig, borrowAndSetNextTimestamp, executeAndTimeTravel } = await loadFixture()
