...
describe('AutomatedLineOfCredit.totalDebt', () => {
  ...
  it('grows over time by unincludedInterest', async () => {
    const { portfolio, borrowAndSetNextTimestamp, mineBlock } = await loadFixture()
    const interest = await borrowAndSetNextTimestamp(parseUSDC(amount), YEAR / 2)
    await mineBlock()
    expect(await portfolio.totalDebt()).to.equal(parseUSDC(amount).add(interest))
  })
  ...
  it('decreases after repay', async () => {
    ...
    expect(await portfolio.totalDebt()).to.equal(parseUSDC(amount / 2).add(interest))
  })
})
