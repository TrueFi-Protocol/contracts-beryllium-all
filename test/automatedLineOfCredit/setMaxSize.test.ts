...
describe('AutomatedLineOfCredit.setMaxSize', () => {
  ...
  it('prevents non-borrower from updating max size', async () => {
    ...
    await expect(portfolio.connect(other).setMaxSize(parseUSDC(amount))).to.be.revertedWith(accessControlMissingRoleRevertMessage(other, MANAGER_ROLE))
  })

  it('prevents setting max size to the existing max size', async () => {
    const { portfolio } = await loadFixture()
    const maxSize = await portfolio.maxSize()
    await expect(portfolio.setMaxSize(maxSize)).to.be.revertedWith('AutomatedLineOfCredit: New max size needs to be different')
  })
