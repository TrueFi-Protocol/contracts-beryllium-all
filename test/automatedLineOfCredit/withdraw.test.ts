...
describe('AutomatedLineOfCredit.withdraw', () => {
  ...
  it('cannot withdraw 0 assets', async () => {
    ...
    await expect(withdraw(wallet, 0))
  ...
  it('cannot withdraw more assets than owned', async () => {
    ...
    await expect(withdraw(wallet, parseUSDC(amount + 1)))
      .to.be.revertedWith('ERC20: burn amount exceeds balance')
  })
  ...
  it('burns LP tokens according to withdraw controller', async () => {
    const { withdraw, wallet, portfolio, mockWithdrawController, parseShares } = await loadFixture()
  ...
  it('can withdraw accrued interest', async () => {
    const { portfolio, wallet, borrowAndRepayWithInterest, withdraw } = await loadFixture()
  ...
  it('cannot withdraw to portfolio', async () => {
    ...
    await expect(withdraw(wallet, 1, portfolio, wallet)).to.be.revertedWith('AutomatedLineOfCredit: Portfolio cannot be the receiver')
  })

  it('cannot withdraw from portfolio', async () => {
    const { portfolio, wallet, withdraw } = await loadFixture()
    await expect(withdraw(wallet, 1, wallet, portfolio)).to.be.revertedWith('AutomatedLineOfCredit: Portfolio cannot be the owner')
  })

  it('reverts if withdrawing exceeds portfolio liquidity', async () => {
    const { portfolio, wallet, withdraw } = await loadFixture()
    await portfolio.borrow(parseUSDC(amount))
    await expect(withdraw(wallet, 1)).to.be.revertedWith('AutomatedLineOfCredit: Operation exceeds portfolio liquidity')
  })
  ...
  it('rounds up', async () => {
    const { portfolio, borrowAndRepayWithInterest, wallet } = await loadFixture()
    ...
    const burnedShares = await portfolio.callStatic.withdraw(parseUSDC(amount), wallet.address, wallet.address)
  ...
  it('cannot withdraw when not allowed by withdrawController', async () => {
    ...
    await expect(withdraw(wallet, 1)).to.be.revertedWith('AutomatedLineOfCredit: Operation not allowed')
  })

  it('updates accruedInterest', async () => {
    const { portfolio, wallet, withdraw, borrowAndSetNextTimestamp } = await loadFixture()
  ...
  it('updates last protocol fee rate', async () => {
    const { wallet, portfolio, withdraw, protocolConfig, protocolFeeRate } = await loadFixture()
  ...
  it('emits a Withdraw event', async () => {
    const { portfolio, wallet, parseShares, borrowAndRepayWithInterest, other, withdraw } = await loadFixture()
    const interest = await borrowAndRepayWithInterest(parseUSDC(amount), YEAR / 2)
  ...
  it('transfers fee', async () => {
    const { portfolio, token, protocolConfig, protocolTreasury, executeAndSetNextTimestamp, withdraw, wallet } = await loadFixture()
  ...
  it('withdraw limit includes fees', async () => {
    const { portfolio, executeAndSetNextTimestamp, protocolConfig, wallet, withdraw } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await executeAndSetNextTimestamp(portfolio.updateAndPayFee(), YEAR / 2)
    await expect(withdraw(wallet, parseUSDC(amount))).to.be.revertedWith('AutomatedLineOfCredit: Operation exceeds portfolio liquidity')
  })

  it('burns more shares after applying fees', async () => {
    const { portfolio, executeAndSetNextTimestamp, protocolConfig, withdraw, wallet } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await executeAndSetNextTimestamp(portfolio.updateAndPayFee(), YEAR / 2)
  ...
  it('emits FeePaid event', async () => {
    const { portfolio, executeAndSetNextTimestamp, protocolConfig, protocolTreasury, wallet, withdraw } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    await executeAndSetNextTimestamp(portfolio.updateAndPayFee(), YEAR / 2)
    await expect(withdraw(wallet, parseUSDC(1))).to.emit(portfolio, 'FeePaid').withArgs(protocolTreasury, parseUSDC(amount * 0.05))
  })
})
