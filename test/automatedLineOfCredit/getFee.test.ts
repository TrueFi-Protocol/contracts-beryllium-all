...
describe('AutomatedLineOfCredit.getFee', () => {
  ...
  it('returns correct amount after year', async () => {
    const { portfolio, deposit, wallet, executeAndTimeTravel } = await loadFixture()
  ...
  it('returns 0 after value drops to 0', async () => {
    const { portfolio, deposit, wallet, executeAndTimeTravel, withdraw, executeAndSetNextTimestamp } = await loadFixture()
    await executeAndSetNextTimestamp(deposit(wallet, parseUSDC(amount)), YEAR / 2)
    await executeAndTimeTravel(withdraw(wallet, parseUSDC(amount - amount * 0.05)), YEAR)
    expect(await portfolio.getFee()).to.equal(0)
  })

  it('returns only most recent update block', async () => {
    const { portfolio, deposit, wallet, getTxTimestamp, executeAndTimeTravel } = await loadFixture()
    const txTimestamp = await getTxTimestamp(deposit(wallet, parseUSDC(amount)))
    await executeAndTimeTravel(deposit(wallet, parseUSDC(amount), wallet, txTimestamp + YEAR / 2), YEAR)
    expect(await portfolio.getFee()).to.equal(parseUSDC((0.95 * amount + amount)).div(10))
  })
