...
describe('AutomatedLineOfCredit.setWithdrawController', () => {
  ...
  it('can only be set by a strategy admin', async () => {
    ...
    await expect(portfolio.connect(other).setWithdrawController(AddressZero))
  ...
  it('must be set to a new strategy', async () => {
    ...
    await expect(portfolio.setWithdrawController(withdrawController.address)).to.be.revertedWith('AutomatedLineOfCredit: New withdraw controller needs to be different')
  })

  it('emits event', async () => {
    const { portfolio, wallet } = await loadFixture()
    const newWithdrawController = await new WithdrawController__factory(wallet).deploy()
    await expect(portfolio.setWithdrawController(newWithdrawController.address)).to.emit(portfolio, 'WithdrawControllerChanged').withArgs(newWithdrawController.address)
  })
})
