import { AutomatedLineOfCredit__factory } from 'build/types'
...
describe('AutomatedLineOfCredit.initialize', () => {
  ...
  it('reverts if minInterestRateUtilizationThreshold is greater than optimumUtilization', async () => {
    const { wallet, token, protocolConfig, controllersAddresses } = await loadFixture()
    const wrongPolyline = {
      ...interestRatePolyline,
      minInterestRateUtilizationThreshold: interestRatePolyline.optimumUtilization,
      optimumUtilization: interestRatePolyline.minInterestRateUtilizationThreshold,
    }
    await expect(deployBehindProxy(new AutomatedLineOfCredit__factory(wallet), protocolConfig.address, 0, token.address, wallet.address, maxSize, wrongPolyline, controllersAddresses, portfolioName, portfolioSymbol))
      .to.be.revertedWith('AutomatedLineOfCredit: Min. Util. <= Optimum Util. <= Max. Util. constraint not met')
  })
  ...
  it('reverts if optimumUtilization is greater than maxInterestRateUtilizationThreshold', async () => {
    ...
    const wrongPolyline = {
      ...interestRatePolyline,
      optimumUtilization: interestRatePolyline.maxInterestRateUtilizationThreshold,
      maxInterestRateUtilizationThreshold: interestRatePolyline.optimumUtilization,
    }
    await expect(deployBehindProxy(new AutomatedLineOfCredit__factory(wallet), protocolConfig.address, 0, token.address, wallet.address, maxSize, wrongPolyline, controllersAddresses, portfolioName, portfolioSymbol))
      .to.be.revertedWith('AutomatedLineOfCredit: Min. Util. <= Optimum Util. <= Max. Util. constraint not met')
  })
  ...
  it('correctly returns minimum interestRate', async () => {
    ...
    expect(await portfolio.interestRate()).to.equal(interestRatePolyline.minInterestRate)
  })
  ...
  it('initializes deposit controller', async () => {
    ...
    expect(await portfolio.depositController()).to.equal(depositController.address)
  })

  it('initializes withdraw controller', async () => {
    const { portfolio, withdrawController } = await loadFixture()
    expect(await portfolio.withdrawController()).to.equal(withdrawController.address)
  })

  it('initializes transfer controller', async () => {
    const { portfolio, transferController } = await loadFixture()
    expect(await portfolio.transferController()).to.equal(transferController.address)
  })

  it('initializes interest rate params', async () => {
    const { portfolio, interestRatePolyline } = await loadFixture()
    expect(await portfolio.interestRateParameters()).to.deep.equal(Object.values(interestRatePolyline))
  })

  it('end date', async () => {
    const { portfolio, portfolioDuration, getTxTimestamp } = await loadFixture()
  ...
  it('sets default admin', async () => {
    const { portfolio, protocolConfig, DEFAULT_ADMIN_ROLE } = await loadFixture()
    const protocolAdmin = await protocolConfig.protocolAdmin()

    expect(await portfolio.hasRole(DEFAULT_ADMIN_ROLE, protocolAdmin)).to.be.true
  })
  ...
  it('sets strategy admin', async () => {
    ...
    expect(await portfolio.getRoleMemberCount(CONTROLLER_ADMIN_ROLE)).to.equal(1)
    expect(await portfolio.hasRole(CONTROLLER_ADMIN_ROLE, wallet.address)).to.be.true
  })
  ...
  it('cannot create a portfolio with zero duration', async () => {
    const { wallet, token, protocolConfig, interestRatePolyline, controllersAddresses } = await loadFixture()
    await expect(deployBehindProxy(new AutomatedLineOfCredit__factory(wallet), protocolConfig.address, 0, token.address, wallet.address, 0, interestRatePolyline, controllersAddresses, 'ALOC', 'ALOC')).to.be.revertedWith('AutomatedLineOfCredit: Cannot have zero duration')
  })
})
