...
describe('AutomatedLineOfCredit.mint', () => {
  ...
  it('transfers assets from sender', async () => {
    ...
    await mint(wallet, parseShares(amount))
    expect(initialAssets.sub(parseUSDC(amount))).to.equal(await token.balanceOf(wallet.address))
  })
  ...
  it('rounds up assets required to mint requested shares', async () => {
    ...
    expect(await token.balanceOf(wallet.address)).to.equal(initialAssets.sub(1))
  })
  ...
  it('requires more assets as portfolio value increases', async () => {
    const { borrowAndRepayWithInterest, portfolio, mint, token, wallet, parseShares } = await loadFixture()
    await mint(wallet, parseShares(amount))
    const interest = await borrowAndRepayWithInterest(parseUSDC(amount), YEAR / 12)
    await mint(wallet, parseShares(amount))
    expect(await token.balanceOf(portfolio.address)).to.equal(parseUSDC(amount).add(interest).mul(2))
  })

  it('updates accruedInterest', async () => {
    const { portfolio, wallet, token, borrowAndSetNextTimestamp, parseShares } = await loadFixture()
    await token.approve(portfolio.address, parseUSDC(amount))
    await portfolio.mint(parseShares(amount).div(2), wallet.address)

    const interest = await borrowAndSetNextTimestamp(parseUSDC(amount).div(2), YEAR / 2)
  ...
  it('does not revert with assets approved from previewMint after interest accrues', async () => {
    const { portfolio, wallet, other, token, borrowAndRepayWithInterest, parseShares, deposit } = await loadFixture()
    ...
    await expect(portfolio.connect(other).mint(parseShares(amount), other.address)).to.not.be.reverted
  })
  ...
  it('reverts with insufficient asset balance', async () => {
    ...
    await expect(mint(other, parseShares(amount)))
      .to.be.revertedWith('ERC20: transfer amount exceeds balance')
  })

  it('reverts when deposits not allowed by controller', async () => {
    const { mint, wallet, mockDepositController, parseShares } = await loadFixture()
  ...
  it('calls onMint with correct arguments', async () => {
    const { mint, wallet, other, parseShares, depositController } = await loadFixture()
    await mint(wallet, parseShares(amount), other)
    expect('onMint').to.be.calledOnContractWith(depositController, [wallet.address, parseUSDC(amount), other.address])
  })

  it('reverts past endDate', async () => {
    const { portfolio, mint, wallet, timeTravel, parseShares } = await loadFixture()
  ...
  it('updates last protocol fee rate', async () => {
    const { portfolio, mint, parseShares, wallet, protocolConfig } = await loadFixture()
  ...
  it('transfers fee', async () => {
    const { mint, token, wallet, deposit, parseShares, protocolConfig, protocolTreasury, getTxTimestamp } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
  ...
  it('mint correct amount with fee', async () => {
    const { portfolio, parseShares, token, other, wallet, deposit, borrowAndSetNextTimestamp, protocolConfig } = await loadFixture()
  ...
  it('transfers correct amount with fee', async () => {
    ...
    expect(parseUSDC(amount * 2).sub(otherTokenBalance)).to.equal(parseUSDC(amount).add(interest).sub(fee))
  })

  it('liquid assets are less than fee', async () => {
    const { portfolio, token, mint, wallet, deposit, protocolConfig, protocolTreasury, getTxTimestamp, calculateInterest } = await loadFixture()
    await deposit(wallet, parseUSDC(amount))
    await protocolConfig.setProtocolFeeRate(1000)
  ...
  it('emits FeePaid event', async () => {
    const { portfolio, mint, wallet, parseShares, protocolConfig, protocolTreasury, getTxTimestamp } = await loadFixture()
    await protocolConfig.setProtocolFeeRate(1000)
    const txTimestamp = await getTxTimestamp(mint(wallet, parseShares(amount)))
    await expect(mint(wallet, 1, wallet, txTimestamp + YEAR / 2)).to.emit(portfolio, 'FeePaid').withArgs(protocolTreasury, parseUSDC(amount * 0.05))
  })

  it('transfers amount specified in controller', async () => {
    const { wallet, parseShares, portfolio, token, mockDepositController } = await loadFixture()
