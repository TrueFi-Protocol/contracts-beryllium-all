...
describe('Upgradeable.revokeDefaultAdminRole', () => {
  ...
  it('reverts if called by non default admin', async () => {
    const { upgradeable, wallet, other, DEFAULT_ADMIN_ROLE } = await loadFixture(upgradeableFixture)
  ...
  it('revokes other default admin', async () => {
    const { upgradeable, other, DEFAULT_ADMIN_ROLE } = await loadFixture(upgradeableFixture)
  ...
  it('revokes its own default admin', async () => {
    const { upgradeable, wallet, DEFAULT_ADMIN_ROLE } = await loadFixture(upgradeableFixture)
