...
export async function automatedLineOfCreditFactoryFixture([protocolOwner]: Wallet[], provider: MockProvider) {
  const portfolioImplementation = await new AutomatedLineOfCredit__factory(protocolOwner).deploy()
  ...
  const factory = await deployBehindProxy(new AutomatedLineOfCreditFactory__factory(protocolOwner), portfolioImplementation.address, protocolConfig.address)
  ...
  async function createPortfolio(
    ...
    const portfolio = new AutomatedLineOfCredit__factory(protocolOwner).attach(portfolioAddress)
