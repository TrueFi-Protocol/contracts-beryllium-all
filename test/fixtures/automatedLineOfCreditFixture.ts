import { AllowAllLenderVerifier__factory, ProtocolConfig__factory } from 'build/types'
import {
...
export const automatedLineOfCreditFixture = ([wallet]: Wallet[], provider: MockProvider) =>
  _automatedLineOfCreditFixture([wallet], interestRatePolyline, provider)

export const automatedLineOfCreditWithInvertedPolylineFixture = ([wallet]: Wallet[], provider: MockProvider) =>
  _automatedLineOfCreditFixture([wallet], invertedInterestRatePolyline, provider)

async function _automatedLineOfCreditFixture([wallet]: Wallet[], _interestRatePolyline: any, provider: MockProvider) {
  const {
    minInterestRate,
  ...
  const whitelistDepositController = await new LegacyWhitelistDepositController__factory(wallet).deploy()
  ...
  const transferController = await new BlockedTransferController__factory(wallet).deploy()

  const protocolTreasury = Wallet.createRandom().address
  const protocolConfig = await deployBehindProxy(new ProtocolConfig__factory(wallet), protocolFeeRate, wallet.address, protocolTreasury, wallet.address)
  ...
  async function deposit(wallet: Wallet, amount: BigNumberish, receiver?: { address: string }, timestamp?: number) {
  ...
  async function mint(wallet: Wallet, shares: BigNumberish, receiver?: { address: string }, timestamp?: number) {
    receiver = receiver || wallet
  ...
  function redeem(wallet: Wallet, shares: BigNumberish, receiver?: { address: string }, owner?: { address: string }) {
    receiver = receiver || wallet
    owner = owner || wallet
    return portfolio.connect(wallet).redeem(shares, receiver.address, owner.address)
  }

  async function borrowAndSetNextTimestamp(amount: BigNumber, time: number = YEAR) {
  ...
  async function borrowAndRepayWithInterest(amount: BigNumber, time: number = YEAR) {
  ...
  async function calculateInterest(amount: BigNumber, duration: number): Promise<BigNumber> {
  ...
  function calculateInterestRate(_utilization: BigNumber): number {
    ...
    if (utilization <= minInterestRateUtilizationThreshold) {
      ...
      return minInterestRate + ((utilization - minInterestRateUtilizationThreshold) * (optimumInterestRate - minInterestRate)) / (optimumUtilization - minInterestRateUtilizationThreshold)
    } else if (utilization <= maxInterestRateUtilizationThreshold) {
      return optimumInterestRate + ((utilization - optimumUtilization) * (maxInterestRate - optimumInterestRate)) / (maxInterestRateUtilizationThreshold - optimumUtilization)
    } else {
      return maxInterestRate
    }
  }

  const mockLenderVerifier = (config: MockLenderVerifierConfig) => _mockLenderVerifier(wallet, portfolio, config)
