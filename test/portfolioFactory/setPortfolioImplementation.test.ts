...
describe('PortfolioFactory.setPortfolioImplementation', () => {
  ...
  it('only default admin can change', async () => {
    const { factory, other, DEFAULT_ADMIN_ROLE } = await loadFixture(portfolioFactoryFixture)
  ...
  it('changes implementation', async () => {
    const { factory, protocolOwner } = await loadFixture(portfolioFactoryFixture)
  ...
  it('reverts when setting the same implementation', async () => {
    const { factory, protocolOwner } = await loadFixture(portfolioFactoryFixture)
    await factory.connect(protocolOwner).setPortfolioImplementation(newImplementation)
    await expect(factory.connect(protocolOwner).setPortfolioImplementation(newImplementation)).to.be.revertedWith('PortfolioFactory: New portfolio implementation needs to be different')
  })
