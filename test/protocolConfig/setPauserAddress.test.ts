...
describe('setPauserAddress', () => {
  ...
  it('only default admin can change pauser address', async () => {
    ...
    await expect(config.connect(otherWallet).setPauserAddress(otherWallet.address))
  ...
  it('prevents from setting the same pauser address', async () => {
    ...
    await expect(config.setPauserAddress(wallet.address))
      .to.be.revertedWith('ProtocolConfig: New pauser address needs to be different')
  })
