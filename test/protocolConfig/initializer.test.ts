...
describe('ProtocolConfig.initializer', () => {
  ...
  it('sets protocol treasury', async () => {
    const { config, protocolTreasury } = await loadFixture(protocolConfigFixture)
  ...
  it('sets pauser as pauser role', async () => {
    const { config, wallet, PAUSER_ROLE } = await loadFixture(protocolConfigFixture)
    expect(await config.hasRole(PAUSER_ROLE, wallet.address)).to.be.true
  })

  it('sets creator as default admin', async () => {
    const { config, wallet, DEFAULT_ADMIN_ROLE } = await loadFixture(protocolConfigFixture)
