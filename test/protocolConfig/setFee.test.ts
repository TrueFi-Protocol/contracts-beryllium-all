...
describe('setFee', () => {
  ...
  it('reverts when new fee is the same', async () => {
    const { config } = await loadFixture(protocolConfigFixture)
    await config.setProtocolFeeRate(100)
    await expect(config.setProtocolFeeRate(100)).to.be.revertedWith('ProtocolConfig: New fee needs to be different')
  })

  it('only default admin can change fee', async () => {
    const { config, protocol, DEFAULT_ADMIN_ROLE } = await loadFixture(protocolConfigFixture)
    await expect(config.connect(protocol).setProtocolFeeRate(100)).to.be.revertedWith(accessControlMissingRoleRevertMessage(protocol, DEFAULT_ADMIN_ROLE))
  })
