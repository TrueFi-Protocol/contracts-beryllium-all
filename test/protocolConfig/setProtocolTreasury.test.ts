...
describe('setProtocolTreasury', () => {
  ...
  it('reverts when new address is the same', async () => {
    const { config, otherWallet } = await loadFixture(protocolConfigFixture)
    await config.setProtocolTreasury(otherWallet.address)
    await expect(config.setProtocolTreasury(otherWallet.address)).to.be.revertedWith('ProtocolConfig: New protocol treasury address needs to be different')
  })

  it('only default admin can change protocol treasury', async () => {
    const { config, otherWallet, DEFAULT_ADMIN_ROLE } = await loadFixture(protocolConfigFixture)
    await expect(config.connect(otherWallet).setProtocolTreasury(otherWallet.address))
      .to.be.revertedWith(accessControlMissingRoleRevertMessage(otherWallet, DEFAULT_ADMIN_ROLE))
  })
